...
/** Macro annotation generates smart constructor for case classes with refined fields. Generated
  * method accumulates all field validation errors to `List[String]`.
  *
  * Example:
  * {{{
  ...
  *   1. Should be `class` or `case class` 2. Constructor should accept `List[String]` or
  *      `List[String] Refined NonEmpty`
  *
